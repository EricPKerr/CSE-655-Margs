    ...
    if hasattr(node, '__iter__'):
        ...
        if tag == '<PROGRAM>' and node[1][0] == '<BLOCK>':
            ...
            # @
                 ...
                 lst[0].tokentype == Token.INPUT:
                return InputStatement(traverse(lst[1]))
...
# import the AST node definitions from astnodes
import astnodes
d = {}
for name in dir(astnodes):
    n = getattr(astnodes, name)
    # if it's a class and it's a subclass of astnodes.ASTNode
    if isinstance(n, type) and issubclass(n, astnodes.ASTNode):
        d[name] = n
globals().update(d)
